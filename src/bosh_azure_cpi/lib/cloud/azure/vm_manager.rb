module Bosh::AzureCloud
  class VMManager
    ...
    def create(uuid, storage_account, stemcell_uri, resource_pool, network_configurator)
      ...
      network_security_group = get_network_security_group()
    ...
    def get_network_subnet(network_configurator)
      ...
      raise "Cannot find the subnet `#{virtual_network_name}/#{subnet_name}' in the resource group `#{resource_group_name}'" if subnet.nil?
      subnet
    end
    ...
    def get_network_security_group()
      ...
      if network_security_group.nil?
        raise "Cannot find the network security group `#{security_group_name}'"
      end
