  ...
  class VMManager
    ...
    def create_availability_set(resource_group_name, availability_set_params)
      ...
      if availability_set.nil?
        ...
        if availability_set[:location] != availability_set_params[:location]
          cloud_error("create_availability_set - the availability set `#{availability_set_name}' already exists, but in a different location. Please delete the availability set or choose another location.")
        else
