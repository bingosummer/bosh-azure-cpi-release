module Bosh::AzureCloud
  class VMManager
    ...
    def create(instance_id, location, image, resource_pool, network_configurator, env, use_managed_disks)
      @logger.info("create(#{instance_id}, #{location}, #{image}, #{resource_pool}, #{network_configurator}, #{env}, #{use_managed_disks})")
      ...
      if @use_managed_disks
        os_disk = @disk_manager2.os_disk(instance_id)
    ...
    def attach_disk(instance_id, disk_name)
      ...
      if @use_managed_disks
        ...
        caching = managed_disk[:tags]['caching']
