...
module Bosh::AzureCloud
  class VMManager
    ...
    def create(bosh_vm_meta, location, vm_props, network_configurator, env)
      ...
      vm_name = _get_bosh_group_name(env)
      instance_id = _build_instance_id(vm_name, bosh_vm_meta, location, vm_props)
    ...
    def _build_instance_id(vm_name, bosh_vm_meta, location, vm_props)
      if @use_managed_disks
        instance_id = InstanceId.create(vm_props.resource_group_name, bosh_vm_meta.agent_id, vm_name)
      else
        storage_account = get_storage_account_from_vm_properties(vm_props, location)
        instance_id = InstanceId.create(vm_props.resource_group_name, bosh_vm_meta.agent_id, vm_name, storage_account[:name])
      end
      instance_id
    end
    ...
    def _get_bosh_group_name(env)
      bosh_group_name = nil
      unless env.nil? || env['bosh'].nil? || env['bosh']['group'].nil?
        bosh_group_name = env['bosh']['group']
        # https://github.com/cloudfoundry-incubator/bosh-azure-cpi-release/issues/209
        # On Azure the length of resource name must be between 1 and 80 characters.
        ...
        if bosh_group_name.size > 80
          bosh_group_name = "az-#{Digest::MD5.hexdigest(bosh_group_name)}-#{bosh_group_name[-40..-1]}"
        end
      end
      bosh_group_name
    end
