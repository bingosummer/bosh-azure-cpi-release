module Bosh::AzureCloud
  class DiskManager2
    ...
    def create_disk(size, location, cloud_properties)
      ...
      disk_name = generate_data_disk_name()
      tags = {
        "user-agent" => "bosh",
        "caching" => caching
      }
    ...
    def create_disk_from_blob(disk_name, blob_uri, location, storage_account_type)
      # DESIGN TODO: The value should be from disk_name. Different from design.
    ...
    def snapshot_disk(disk_name, metadata)
      ...
      snapshot_name = generate_data_disk_name()
      tags = {
        "original" => disk_name
      }
      snapshot_params = {
      ...
      @logger.info("Start to create a snapshot from a managed disk `#{disk_name}'")
