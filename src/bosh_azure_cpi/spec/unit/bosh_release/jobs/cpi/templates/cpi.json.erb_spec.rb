  ...
  context 'when parsing the azure property' do
    ...
    context 'when the ssh public key is not provided' do
      ...
      it 'raises an error of missing ssh_public_key' do
        expect { subject }.to raise_error(/ssh_certificate has been replaced by ssh_public_key/)
      end
    end

    context 'when the windows properties are provided' do
      context 'username and passwrod are provided' do
        let(:username) { 'fake-windows-username' }
        let(:password) { 'fake-windows-password' }
        before do
          manifest['properties']['azure']['windows'] = {
            'username' => username,
            'password' => password
          }
        end

        it 'parses the username and password' do
          expect(subject['cloud']['properties']['azure']['windows']['username']).to eq(username)
          expect(subject['cloud']['properties']['azure']['windows']['password']).to eq(password)
        end
      end

      context 'username and passwrod are both not provided' do
        before do
          manifest['properties']['azure']['windows'] = {
            'username' => '',
            'password' => ''
        ...
        it 'raises an error of missing windows username and password' do
          expect { subject }.to raise_error(/Both "username" and "password" must be set for Windows/)
        end
      end

      context 'username is not provided' do
        let(:password) { 'fake-windows-password' }
        before do
          manifest['properties']['azure']['windows'] = {
            'username' => '',
      ...
      context 'passwrod is not provided' do
        let(:username) { 'fake-windows-username' }
        before do
          manifest['properties']['azure']['windows'] = {
            'username' => username,
    ...
    context 'when the environment is AzureStack' do
      ...
      context 'when all the required properties are provided' do
        before do
          manifest['properties']['azure']['azure_stack_domain'] = 'fake-domain'
          manifest['properties']['azure']['azure_stack_authentication'] = 'fake-authentication'
          manifest['properties']['azure']['azure_stack_resource'] = 'fake-resource'
        end

        it 'parses the AzureStack properties' do
          expect(subject['cloud']['properties']['azure']['azure_stack_domain']).to eq('fake-domain')
          expect(subject['cloud']['properties']['azure']['azure_stack_authentication']).to eq('fake-authentication')
          expect(subject['cloud']['properties']['azure']['azure_stack_resource']).to eq('fake-resource')
          expect(subject['cloud']['properties']['azure']['azure_stack_endpoint_prefix']).to eq('api')
        end
      end
      ...
      context 'when maximal properties are provided' do
        before do
          ...
          manifest['properties']['azure']['azure_stack_authentication'] = 'fake-authentication'
          manifest['properties']['azure']['azure_stack_resource'] = 'fake-resource'
          manifest['properties']['azure']['azure_stack_endpoint_prefix'] = 'fake-endpoint-prefix'
        end
        ...
        it 'parses the AzureStack properties' do
          ...
          expect(subject['cloud']['properties']['azure']['azure_stack_authentication']).to eq('fake-authentication')
          expect(subject['cloud']['properties']['azure']['azure_stack_resource']).to eq('fake-resource')
          expect(subject['cloud']['properties']['azure']['azure_stack_endpoint_prefix']).to eq('fake-endpoint-prefix')
        end
      end

      context 'when azure_stack_domain is invalid' do
        before do
          manifest['properties']['azure']['azure_stack_authentication'] = 'fake-authentication'
          manifest['properties']['azure']['azure_stack_resource'] = 'fake-resource'
        end
        ...
        context 'when azure_stack_domain is nil' do
          before do
            manifest['properties']['azure']['azure_stack_domain'] = nil
          end

          it 'raises an error of missing azure_stack_domain' do
            expect { subject }.to raise_error(/You must provide a domain for AzureStack/)
          end
        end

        context 'when azure_stack_domain is empty' do
          before do
            manifest['properties']['azure']['azure_stack_domain'] = ''
          end

          it 'raises an error of missing azure_stack_domain' do
            expect { subject }.to raise_error(/The AzureStack domain can not be empty/)
          end
        end
      end

      context 'when azure_stack_authentication is invalid' do
        before do
          manifest['properties']['azure']['azure_stack_domain'] = 'fake-domain'
          manifest['properties']['azure']['azure_stack_resource'] = 'fake-resource'
        end
        ...
        context 'when azure_stack_authentication is nil' do
          before do
            manifest['properties']['azure']['azure_stack_authentication'] = nil
          end

          it 'raises an error of missing azure_stack_authentication' do
            expect { subject }.to raise_error(/You must provide a authentication for AzureStack/)
          end
        end

        context 'when azure_stack_authentication is empty' do
          before do
            manifest['properties']['azure']['azure_stack_authentication'] = ''
          end

          it 'raises an error of missing azure_stack_authentication' do
            expect { subject }.to raise_error(/The AzureStack authentication can not be empty/)
          end
        end
      end

      context 'when azure_stack_resource is invalid' do
        before do
          manifest['properties']['azure']['azure_stack_domain'] = 'fake-domain'
          manifest['properties']['azure']['azure_stack_authentication'] = 'fake-authentication'
        end

        context 'when azure_stack_resource is nil' do
          before do
            manifest['properties']['azure']['azure_stack_resource'] = nil
          end

          it 'raises an error of missing azure_stack_resource' do
            expect { subject }.to raise_error(/You must provide a resource for AzureStack/)
          end
        end

        context 'when azure_stack_resource is empty' do
          before do
            manifest['properties']['azure']['azure_stack_resource'] = ''
          end

          it 'raises an error of missing azure_stack_resource' do
            expect { subject }.to raise_error(/The AzureStack resource can not be empty/)
          end
        end
      end
    end
