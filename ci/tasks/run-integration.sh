...
export BOSH_AZURE_DEFAULT_SECURITY_GROUP=$(echo ${metadata} | jq -e --raw-output ".default_security_group_name")
...
export AZURE_STORAGE_ACCESS_KEY=$(azure storage account keys list ${BOSH_AZURE_STORAGE_ACCOUNT_NAME} -g ${BOSH_AZURE_DEFAULT_RESOURCE_GROUP_NAME} --json | jq '.[0].value' -r)
