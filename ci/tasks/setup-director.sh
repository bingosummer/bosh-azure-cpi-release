...
DIRECTOR=$(azure network public-ip show ${AZURE_GROUP_NAME_2} AzureCPICI-bosh --json | jq '.ipAddress' -r)
...
- name: public
  type: vip
  cloud_properties:
    resource_group_name: $AZURE_GROUP_NAME_2
- name: private
  type: manual
  subnets:
  - range: 10.0.0.0/24
...
- name: bosh
  ...
  properties:
    ...
    azure: &azure
      ...
      resource_group_name: $AZURE_GROUP_NAME_1
