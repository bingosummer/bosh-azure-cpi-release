...
# Check if the resource group already exists
echo "azure group list | grep ${AZURE_GROUP_NAME_2}"
azure group list | grep ${AZURE_GROUP_NAME_2}

if [ $? -eq 0 ]
then
  echo "azure group delete ${AZURE_GROUP_NAME_2}"
  azure group delete ${AZURE_GROUP_NAME_2} --quiet
  echo "waiting for delete operation to finish..."
  # Wait for the completion of deleting the resource group
  azure group show ${AZURE_GROUP_NAME_2}
  while [ $? -eq 0 ]
  do
    azure group show ${AZURE_GROUP_NAME_2} > /dev/null 2>&1
    echo "..."
  done
fi
...
echo azure group create ${AZURE_GROUP_NAME_2} ${AZURE_REGION_SHORT_NAME}
azure group create ${AZURE_GROUP_NAME_2} ${AZURE_REGION_SHORT_NAME}
...
EOF
azure group deployment create ${AZURE_GROUP_NAME_2} --template-file ./bosh-cpi-release/ci/assets/azure/network.json --parameters-file ./network-parameters.json
